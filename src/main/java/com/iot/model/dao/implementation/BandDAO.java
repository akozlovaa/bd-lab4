...
public class BandDAO implements GeneralDAO<Band> {
    ...
    private static final String CREATE = "INSERT  spotify.band "
            + "(name, num_of_artists) VALUES (?,?)";
    private static final String UPDATE = "UPDATE spotify.band"
            + " SET name=?,num_of_artists=? WHERE id=?";
    ...
    public void create(Band band) throws SQLException {
        try (PreparedStatement statement = DataBaseConnector.getConnection().prepareStatement(CREATE)) {
            ...
            statement.setInt(2, Integer.valueOf(band.getNum_of_artists()));
